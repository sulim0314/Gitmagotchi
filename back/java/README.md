...
```bash
java$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
java$ sam local start-api
java$ curl http://localhost:3000/
```
...
```bash
java$ sam logs -n HelloWorldFunction --stack-name java --tail
```
